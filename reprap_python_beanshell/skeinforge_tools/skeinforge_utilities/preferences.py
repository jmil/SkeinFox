...
globalPreferencesDialogListTable = {}
globalProfileSaveListenerListTable = {}
globalCloseListTables = [ globalPreferencesDialogListTable, globalProfileSaveListenerListTable ]
...
def getLowerNameSetHelpTitleWindowPosition( displayPreferences, fileNameHelp ):
	...
	for preference in displayPreferences.archive:
		preference.displayPreferences = displayPreferences
...
def quitWindows():
	"Quit all windows."
	global globalPreferencesDialogListTable
	globalPreferencesDialogValues = euclidean.getListTableElements( globalPreferencesDialogListTable )
...
def updateProfileSaveListeners():
	...
	for globalProfileSaveListeners in euclidean.getListTableElements( globalProfileSaveListenerListTable ):
...
def writePreferencesPrintMessage( archivablePreferences ):
	"Set the preferences to the dialog then write them."
	writePreferences( archivablePreferences )
	print( archivablePreferences.title.lower().capitalize() + ' have been saved.' )
...
class AddListboxSelection:
	...
	def addToMenu( self, preferencesMenu ):
		"Do nothing because the add listbox selection does not place itself on a menu."
		pass
...
class AddProfile:
	...
	def addToMenu( self, preferencesMenu ):
		"Do nothing because the add profile does not place itself on a menu."
		pass

	def addToPreferenceTable( self, preferenceTable ):
		"Do nothing because the add profile is not archivable."
		pass
...
class StringPreference:
	...
	def addToMenu( self, preferencesMenu ):
		"Add this to the preferences menu."
		preferencesMenu.add_command( label = getTitleFromName( self.name ) + '..', command = self.openPreferenceDialog )
	...
	def openPreferenceDialog( self ):
		...
		PreferenceDialog( self, Tkinter.Tk() ).completeDialog()
...
class CloseListener:
	"A class to listen to link a window to the global preferences dialog list table."
	def __init__( self, lowerName, window, closeFunction = None ):
		"Add the window to the global preferences dialog list table."
		self.closeFunction = closeFunction
		self.lowerName = lowerName
		self.window = window
		self.shouldWasClosedBeBound = True
		global globalPreferencesDialogListTable
		euclidean.addElementToListTable( window, lowerName, globalPreferencesDialogListTable )
	...
	def wasClosed( self, event ):
		...
		for globalCloseListTable in globalCloseListTables:
			euclidean.removeElementFromListTable( self.window, self.lowerName, globalCloseListTable )
...
class DisplayToolButtonBesidePrevious:
	...
	def addToMenu( self, preferencesMenu ):
		"Do nothing because the button does not place itself on a menu."
		pass
...
#		if self.displayPreferencesLowerName in globalPreferencesDialogTable:
		self.value = self.getLowerName() in globalPreferencesDialogListTable
...
class FileHelpMenuBar:
	...
	def completeMenu( self, closeFunction, lowerName ):
		...
		euclidean.addElementToListTable( self, lowerName, globalProfileSaveListenerListTable )
...
class LabelDisplay:
	...
	def addToMenu( self, preferencesMenu ):
		"Do nothing because the label display does not place itself on a menu."
		pass
...
class MenuButtonDisplay:
	...
	def addToPreferenceTable( self, preferenceTable ):
		"Do nothing because the menu button is not archivable."
		pass
	...
	def getMenuButton( self, name, preferencesDialog ):
		"Get the menu button."
		...
		self.label = Tkinter.Label( preferencesDialog.root, text = self.name )
		self.label.grid( row = preferencesDialog.row, column = 0, columnspan = 3, sticky = Tkinter.W )
		self.menuButton = Tkinter.OptionMenu( preferencesDialog.root, self.radioVar, self.optionList )
		self.menuButton.grid( row = preferencesDialog.row, column = 3, columnspan = 2, sticky = Tkinter.W )
	...
	def openPreferenceDialog( self ):
		"Open the preference dialog."
		self.radioVar = None
		preferenceDialog = PreferenceDialog( self, Tkinter.Tk() )
		for menuRadio in self.menuRadios:
			menuRadio.addToDialog( preferenceDialog )
		preferenceDialog.completeDialog()

	def setToDisplay( self ):
		"Do nothing because the menu button display is not archivable."
		pass
...
class MenuRadio( BooleanPreference ):
	...
	def addToDialog( self, preferencesDialog ):
		"Add this to the dialog."
		menu = self.menuButtonDisplay.getMenuButton( self.name, preferencesDialog ).menu
	...
	def addToMenu( self, preferencesMenu ):
		"Do nothing because the menu radio does not place itself on a menu."
		pass
...
class ProfileList:
	...
	def addToMenu( self, preferencesMenu ):
		"Do nothing because the profile list does not place itself on a menu."
		pass
...
class ToolDialog:
	...
	def addPluginToMenu( self, directoryPath, menu, name ):
		"Add the display command to the menu."
		self.getDisplayFromFolderPath( directoryPath, name )
		menu.add_command( label = name.capitalize(), command = self.display )

	def display( self ):
		"Display the tool preferences dialog."
		global globalPreferencesDialogListTable
		if self.name.lower() in globalPreferencesDialogListTable:
			for toolPreferencesDialog in globalPreferencesDialogListTable[ self.name.lower() ]:
			...
			return
		pluginModule = gcodec.getModuleWithDirectoryPath( self.directoryPath, self.name )
		if pluginModule != None:
			getDisplayedDialogFromConstructor( pluginModule.getPreferencesConstructor() )

	def getDisplayFromFolderPath( self, directoryPath, name ):
...
class WindowPosition( StringPreference ):
	...
	def addToMenu( self, preferencesMenu ):
		"Do nothing because the window position does not place itself on a menu."
		pass
...
class PreferenceDialog:
	def __init__( self, displayPreference, root ):
		"Create display preference dialog."
		self.closeListener = CloseListener( displayPreference.name.lower(), self )
		self.displayPreference = displayPreference
		self.root = root
		self.row = 0
		root.withdraw()
		root.title( getTitleFromName( displayPreference.name ) )

	def completeDialog( self ):
		"Complet the display preference dialog."
		self.displayPreference.addToDialog( self )
	...
	def save( self ):
		"Set the preferences to the dialog then write them."
		self.displayPreference.setToDisplay()
		writePreferencesPrintMessage( self.displayPreference.displayPreferences )
		if self.displayPreference.updateFunction != None:
			self.displayPreference.updateFunction()
...
class PreferencesDialog:
	def __init__( self, displayPreferences, root ):
		...
		self.executables = []
		self.isFirst = ( len( globalPreferencesDialogListTable.keys() ) == 0 )
		self.root = root
	...
	def close( self ):
		"The dialog was closed."
		global globalPreferencesDialogListTable
		if self.isFirst and len( globalPreferencesDialogListTable.keys() ) > 1:
			self.root.iconify()
	...
	def save( self ):
		...
		writePreferencesPrintMessage( self.displayPreferences )
