...
def getCraftedTextFromText( gcodeText, raftlessRepository = None ):
	...
	if raftlessRepository == None:
		raftlessRepository = preferences.getReadRepository( RaftlessRepository() )
...
class RaftlessRepository:
	...
	def __init__( self ):
		...
		self.fileNameInput = preferences.Filename().getFromFilename( interpret.getGNUTranslatorGcodeFileTypeTuples(), 'Open File for Raftless', self, '' )
		self.activateRaftless = preferences.BooleanPreference().getFromValue( 'Activate Raftless', self, False )
		self.firstPerimeterFeedrateOverFeedrate = preferences.FloatPreference().getFromValue( '1st Perimeter Feed Rate over Feed Rate (ratio):', self, 0.5 )
		self.firstPerimeterFlowrateOverFlowrate = preferences.FloatPreference().getFromValue( '1st Perimeter Flow Rate over Flow Rate (ratio):', self, 1. )
		self.addExtrusionIntro = preferences.BooleanPreference().getFromValue( 'Add Extrusion Intro:', self, True )
		self.absMaxXIntro = preferences.FloatPreference().getFromValue( 'Extrusion Intro Max X Absolute (mm):', self, 40.0 )
		self.absMaxYIntro = preferences.FloatPreference().getFromValue( 'Extrusion Intro Max Y Absolute (mm):', self, 40.0 )
		#Create the archive, title of the execute button, title of the dialog & preferences fileName.
		self.executeTitle = 'Raftless'
		preferences.setHelpPreferencesFileNameTitleWindowPosition( self, 'skeinforge_tools.craft_plugins.raftless.html' )
